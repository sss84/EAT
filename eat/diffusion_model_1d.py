...
from eat.version import __version__
...
class GaussianDiffusion1D(Module):
    ...
    def _get_frequency_mask(self, H, W, device):
        key = (H, W, device)
        ...
        if key not in self._freq_mask_cache:
            ...
            mask = torch.exp(-self.frequency_alpha * radius ** 2)
            self._freq_mask_cache[key] = mask[None, None]

        return self._freq_mask_cache[key]

    def _frequency_attenuation(self, x, alpha=None):
        """
        x: [B, C, H, W] offset
        """
        if alpha is None:
            alpha = self.frequency_alpha
        ...
        # mask
        mask = self._get_frequency_mask(H, W, device)
        ...
        x_filtered = torch.fft.ifft2(fft, dim=(-2, -1)).real
